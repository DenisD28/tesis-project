...
export default function SelectForm({dataSelect}: {dataSelect: SelectFormProps}) {
  return (
    <div className="flex justify-center items-center flex-col p-2">
        ...
        <select onChange={(e) => dataSelect.fnChange(e.currentTarget.value)} className="w-full h-10 rounded border-2 border-[#ddd] px-4 font-medium bg-slate-100 text-[#555]" name={dataSelect.name} id={dataSelect.name}>
