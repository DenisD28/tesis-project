...
export default function InputsForm({ DataInputs }: { DataInputs: InputsFormProps }) {
  return (
    <div className="flex justify-center items-center flex-col p-2">
      ...
      <input onChange={(e) => {DataInputs.fnChange(e.currentTarget.value)}} className="w-full h-10 rounded border-2 border-[#ddd] px-4 font-medium bg-slate-100 text-[#555]" type={DataInputs.type} name={DataInputs.name} placeholder={DataInputs.placeholder} min={0} />
    </div>
  )
}
